...
const prisma = new PrismaClient();
 ...
 export  async function validate(request,response,next){
    ...
    if (!firstName) {
        return response.status(400).json({success:false,message:"first name is required"});}
        
    if (!lastName) {
        return response.status(400).json({success:false,message:"last name is required"});}
    ...
    if (!password) {
        ...
        if(userWithEmail) 
            return response.status(400).json({success:false, message: "Email already taken"})
    next()
   }     
