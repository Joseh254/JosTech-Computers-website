...
export async function createMotherboard(request, response) {
  console.log(request.body);
    try {
      const {motherboardName,motherboardDescription,motherboardPrice,motherboardImage} = request.body;
      const motherboardexists = await prisma.motherboards.findFirst({
        where:{motherboardName:motherboardName}
      })
      if(motherboardexists){
        return response.status(400).json({success:false, message:"A product with the same name already exists"})
